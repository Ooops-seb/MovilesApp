...
class MiAgenda extends StatefulWidget {
  final String userId;
  const MiAgenda({required this.userId, Key? key}) : super(key: key);

  @override
  State<MiAgenda> createState() => _MiAgendaState();
}

class _MiAgendaState extends State<MiAgenda> {
  ...
  Future<void> loadBookings() async {
    ...
    if (bookings != null) {
      for (var booking in bookings) {
        booking.laboratory = await _laboratoryService.getLaboratory(booking.labId);
        booking.course = await _courseService.getCourse(booking.courseId.toString());
        booking.user = await _userService.getUserById(booking.course!.userId);
        booking.schedule = await _scheduleService.getSchedule(booking.scheduleId.toString());
      }
      bookings = bookings.where((booking) => booking.course != null && booking.course!.userId == widget.userId).toList();
    }
